...
@Autonomous(name="Blue Advance V3", group="Linear Opmode")      // This is the Displat Name on the Driver Hub 

public class BlueAdvanceV3 extends LinearOpMode {              // Make sure to rename the file to match the file name.
...
private ElapsedTime     runtime = new ElapsedTime();
    ...
    public void runIntakeMovement(double distance, long timeInMillis, intakeMode mode) {
        // Replace the logic with runParallelWithTimeout
        runParallelWithTimeout(
            () -> {
                ...
                try {
                    Thread.sleep(timeInMillis); // Allow intake/outtake to run for the specified time
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                intake.setPower(0.2); // Stop intake/outtake
                runArmToPos(armMode.HOVER);
            },
            () -> drivetrainAuto.profiledDriveStraight(distance), // Drivetrain operation
            timeInMillis + 500 // Timeout slightly longer than intake duration
        );
    }
    ...
    // Add a method to handle parallel operations with timeout
    public void runParallelWithTimeout(Runnable operation1, Runnable operation2, long timeoutMillis) {
        ...
        // Stop threads if timeout occurs
    ...
    public void runOpMode() {
        ...
        drivetrainAuto.profiledDriveDiagonal(2.0, "FR");     // Move 2 meters diagonally forward-right | "FR" "FL" "RR" "RL"
        ...
        // AUTONOMOUS INSTRUCTIONS BELOW:      
         ...
         Example: Run intake and drivetrain movement in parallel with a timeout
         runParallelWithTimeout(
             () -> runIntake(2000), // Intake operation
             () -> drivetrainAuto.profiledDriveStraight(1.0), // Drivetrain operation
        ...
        */ 
        ...
        drivetrainAuto.profiledDriveDiagonal(0.85, "FL"); 

            specimen.setTargetPosition(specimenHighClip);
            specLServo.setPower(-1.0);   // OUTTAKE
            specRServo.setPower(1.0);
            sleep(500);
            drivetrainAuto.profiledDriveStraight(-0.06);    
            drivetrainAuto.profiledDriveDiagonal(0.065, "BL");
            specLServo.setPower(0.0); 
            specRServo.setPower(0.0);
            specimen.setTargetPosition(specimenWall);

        drivetrainAuto.profiledTurnHeading(-56.0); // TOWARDS FLOOR SAMPLE 
        drivetrainAuto.profiledDriveStraight(-0.05);   
        runIntakeMovement(0.6, 1000, intakeMode.INTAKING); // INTAKE FLOOR SAMPLE
        drivetrainAuto.profiledTurnHeading(-115.0); // TOWARDS OBV
        runOuttake(800); // OUTTAKES 
        runArmToPos(armMode.HOME); // ARM RESET
        drivetrainAuto.profiledTurnHeading(57.0);            
        drivetrainAuto.profiledDriveSideways(-0.5);  
        drivetrainAuto.profiledDriveStraight(0.2);   
        drivetrainAuto.profiledDriveSideways(1.0);  
